function bill = cost_eMPCComp3_cert(Nh_add, start, month, num_days, meta_data)
    ...
    % ========== DEFINE OPTIMIZATION PROBLEM AND RUN MPC LOOP ==========
        ...
        w = 2;
        a_tel = ((t+N_Vec(t)-1)/Steps)^w;    

        f = [c(t:t+N_Vec(t)-1)'*10e-6*(1/H_split) ; zeros(N_Vec(t)*n,1); 10e8*ones(2*N_Vec(t),1); cp*a_tel];
    ...
    save(['Heatpump\Benchmark\eMPCComp3_cert\cost_eMPCComp3_cert_',num2str(Nh_add),'h_', month, '_FULL'], vars{:}) 
