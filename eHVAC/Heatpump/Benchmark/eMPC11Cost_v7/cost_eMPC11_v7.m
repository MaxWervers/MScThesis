function bill = cost_eMPC11_v7(Nh_add, start, month, num_days, meta_data)
    ...
    filename = ['cost_to_go_v3_48h_', month, '.mat'];
    ...
    for pred_idx = 1:num_preds
        ...
        for t = 1:Steps-1
            ...
            size_opt = N_vec_ext(t)*(mc+n+2) + 2; % lenght of optimization vector
            ...
            tau = min(1 + (t-1+N_vec_ext(t))/(H_split*24), num_days); 
            ...
            f = [c_real_pred' * 10e-6 * (1/H_split); zeros(N_vec_ext(t)*n, 1); 10e8 * ones(2*N_vec_ext(t), 1); 1; 0];
            e_eq = gen_e(Ad, Bd_d, d_pred, N_vec_ext(t), X_sim(:,t));
            [E_in, e_in] = construct_soft_constraints_HPeMPC9(N_vec_ext(t), mc, n, Tmin_air(t+1:t+N_vec_ext(t)), Tmax_air(t+1:t+N_vec_ext(t)), Tmax_w, 0, u_prev_max, a_i{tau}', b_i{tau}');
            ...
            U_sim(:,t) = sol(1);
            u_prev_max = max(u_prev_max,sol(1));
    ...
    save(['Heatpump\Benchmark\eMPC11Cost_v7\cost_HPeMPC11_v7_',num2str(Nh_add),'h_add_', month, '_FULL'], vars{:})
